                      ...
                      _data.delete('checks', checkId, function(err) {
                        ...
                        if(checksDeleted == checksToDelete) {
                          if(!deletionErrors){
                            ...
                            callback(500, {'Error':'There was a cock up here'});
                          }
                        }
                      });
                    });
...
handlers._checks.get = function(data, callback) {
  ...
  if(id){
    ...
    callback(400,{'Error':'Missing required feild, or invalid feilds'});
  };
};
                ...
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData){
                    ...
                    if(checkPosition > -1) {
                      ...
                      callback(500, {'Error':'Could not find the check on the users'});
                    };
                  } else {
                    callback(500,{'Error' : 'Could not find user who created the check. Could not remove the check from the users check object.'});
                  }
                });
              } else {
